...
a <- ggplot(data = fig5data[[8]],aes(x=Prediction, y=Reference), fill=value) + geom_tile(aes(fill=value)) +
  scale_y_discrete(limits = rev(levels(fig5data[[8]]$Reference)), labels=rev(Eline_name), position = "right") +
  scale_x_discrete(labels=Eline_name) +labs(y ="True") + geom_text(label=fig5data[[8]]$value)

b<- ggplot() +geom_blank() + theme_void() + annotate("text", label="ANN", x = 5.5, y=3)
  
tmp <- grid.arrange(a, b, ncol = 1, heights = c(9,1))                             
```

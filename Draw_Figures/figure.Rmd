...
fig2_r <- ggplot(data=melted_fig2b, aes(X1, X2, fill=value)) + 
      ...
      scale_y_discrete(position = "right", labels = parse(text =TeX(unique(melted_fig2b$X1)))) + 
...
# TODO redraw pie chart(figure3) to cumulative bar chart.
```{r, message=FALSE, warning=FALSE}
require(ggplot2)
require(gridExtra)
...
fig3_1 <- ggplot(Bpro, aes(x = set, y = probability, fill = neuron)) + geom_bar(stat="identity") + 
...
fig3_2 <- ggplot(Epro, aes(x = set, y = probability, fill = neuron)) + geom_bar(stat="identity") + 
  theme_minimal() + scale_fill_brewer(type = "seq", palette = "Spectral") +
  labs(x = "", fill  = "excitatory line")
...
fig3_3 <- ggplot(Ipro, aes(x = set, y = probability, fill = neuron)) + geom_bar(stat="identity") + 
  theme_minimal() + scale_fill_brewer(type = "seq", palette = "Spectral") +
  labs(x = "", fill = "inhibitory line")

fig3 <- grid.arrange(fig3_1, fig3_2, fig3_3, ncol = 1)

ggsave("fig3.tiff", fig3, width = 7, height = 10, units = "in", dpi = 300)
```

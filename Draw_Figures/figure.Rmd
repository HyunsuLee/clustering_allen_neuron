...
for(i in 1:3) {
  if (models[i] == "ANN") {
    for(j in 1:4) {
      ...
      auc_ANN <- cbind.data.frame(auc_ANN, auc_each)
      ...
      for(j in 1:4) {
        ...
        auc_each <- rbind.data.frame(auc_each, models[i], input[j])
        colnames(auc_each) <- modelname
        auc_regfr <- cbind.data.frame(auc_regfr, auc_each)
        }
  }
}
...
auc_binary <- cbind.data.frame(auc_ANN[2:5], auc_regfr[2:9])
...
tmp<-predict(E.reg.full.nocv, newdata = Etest) %>% confusionMatrix(Etest$transgenic_line)
fig5data[[1]] <- melt(tmp$table)
fig5[[1]] <- ggplot(data = fig5data[[1]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-predict(E.rf.full.nocv, newdata = Etest) %>% confusionMatrix(Etest$transgenic_line)
fig5data[[2]] <- melt(a$table)
fig5[[2]] <- ggplot(data = fig5data[[2]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-confusionMatrix(as.factor(E.ANN.full[,1]+1),as.factor(as.integer(Etest$transgenic_line)))
fig5data[[3]] <- melt(a$table)
fig5[[3]] <- ggplot(data = fig5data[[3]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))



a<-predict(E.reg.long.nocv, newdata = Etest.l) %>% confusionMatrix(Etest$transgenic_line)
fig5data[[4]] <- melt(a$table)
fig5[[4]] <- ggplot(data = fig5data[[4]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-as.integer(predict(E.rf.long.nocv, newdata = Etest.l)) %>% confusionMatrix(as.integer(Etest$transgenic_line))
fig5data[[5]] <- melt(a$table)
fig5[[5]] <- ggplot(data = fig5data[[5]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-confusionMatrix(E.ANN.long[,1]+1,as.integer(Etest$transgenic_line))
fig5data[[6]] <- melt(a$table)
fig5[[6]] <- ggplot(data = fig5data[[6]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))


a<-as.integer(predict(E.reg.short.nocv, newdata = Etest.s)) %>% confusionMatrix(as.integer(Etest$transgenic_line))
fig5data[[7]] <- melt(a$table)
fig5[[7]] <- ggplot(data = fig5data[[7]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-as.integer(predict(E.rf.short.nocv, newdata = Etest.s)) %>% confusionMatrix(as.integer(Etest$transgenic_line))
fig5data[[8]] <- melt(a$table)
fig5[[8]] <- ggplot(data = fig5data[[8]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-confusionMatrix(E.ANN.short[,1]+1,as.integer(Etest$transgenic_line))
fig5data[[9]] <- melt(a$table)
fig5[[9]] <- ggplot(data = fig5data[[9]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))


a<-as.integer(predict(E.reg.ramp.nocv, newdata = Etest.r)) %>% confusionMatrix(as.integer(Etest$transgenic_line))
fig5data[[10]] <- melt(a$table)
fig5[[10]] <- ggplot(data = fig5data[[10]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-as.integer(predict(E.rf.ramp.nocv, newdata = Etest.r)) %>% confusionMatrix(as.integer(Etest$transgenic_line))
fig5data[[11]] <- melt(a$table)
fig5[[11]] <- ggplot(data = fig5data[[11]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))

a<-confusionMatrix(E.ANN.ramp[,1]+1,as.integer(Etest$transgenic_line))
fig5data[[12]] <- melt(a$table)
fig5[[12]] <- ggplot(data = fig5data[[12]],aes(x=Prediction, y=ordered(Reference,levels=rev(Reference)), fill=value))


ggplot(data = fig5data[[2]],aes(x=Prediction, y=Reference), fill=value) + geom_tile(aes(fill=value)) +
  scale_y_discrete(limits = rev(levels(fig5data[[2]]$Reference)), labels=rev(Eline_name), position = "right") +
  scale_x_discrete(labels=Eline_name) +labs(y ="True")
